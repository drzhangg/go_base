...
func main() {
	...
	customRepo := &repo.Entry{
		Name:                  "custom-repo1",
		URL:                   helm_repo,
	}

	chartRepo,err := repo.NewChartRepository(customRepo,getter.All(settings))
	if err != nil {
	...
	fmt.Println("cache::",settings.RepositoryCache)

	// 加载仓库索引文件
	indexFilePath := settings.RepositoryCache + "/" + customRepo.Name + "-index.yaml"
